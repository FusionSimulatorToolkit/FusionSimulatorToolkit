...
my $usage = "usage: $0 fusions.fasta output_prefix read_length\n\n";
...
sub sim_fusion_reads {
    ...
    while ($count_split == 0) {
        ...
        if ($try_counter > 100) {
            print STDERR "-no fusion reads possible here\n";
            last;
        }
    }
...
sub sim_rnaseq_reads {
    ...
    my $outer_distance = min(500, int(length($sequence)/2));
   

    my $cmd = "wgsim-trans $tmp_fa_file $left_fq_file $right_fq_file -d $outer_distance -N $num_reads -1 $read_len -2 $read_len -e 0.001 -r 0"; 
    &process_cmd($cmd);
